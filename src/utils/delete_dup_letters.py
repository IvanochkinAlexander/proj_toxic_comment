...
train_test= pd.read_json('../../data/interim/train_test_lemmed.json')
print ('loaded')
# train_test = train_test
train_test['comment_text_splited'] = train_test['comment_text_proc'].apply(lambda x : x.split(' '))
...
def delete_dup_letters (col):
    try:
        ...
        for w_ind in range(len(col)):
            if col[w_ind] not in correct_words:
...
train_test.to_json('../../data/interim/train_test_no_dup.json')
